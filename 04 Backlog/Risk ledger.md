...
|----|------|----------|--------|--------------------|
| R1 | **Account deletion undefined** — Devise `registerable` ships a live `DELETE /users`; `dependent: :destroy` would orphan workshops (breaks the lifetime Ownership invariant), erase employment history (breaks append-only), and FK-crash on any fired invitation | high | **open → [[Decisions]] ADR-009 pending** (leaning: refuse with edges/history; bare users free; anonymize = PDPA path) | settle ADR-009's edges (trapped-owner, law-vs-ADR, PDPA trigger), then ~10 lines of guard |
