...
| Rejected | Why |
|---|---|
| STI user subclasses / universal `accounts` table with `account_type` | Roles live on edges ([[Architecture laws]] #1); User stays a thin login |
...
| Dual-writing job copies to the owner side | Owner visibility is a query, not a second copy ([[Architecture laws]] #3) |
